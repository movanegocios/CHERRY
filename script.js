...
(function () {
  ...
  function renderHero() {
    ...
    setText(".solution .btn-with-logo span, .boost .btn-with-logo span", t.hero.cta_primary);
    setAttr(".solution .btn-with-logo", "aria-label", t.hero.cta_primary);
    setAttr(".boost .btn-with-logo", "aria-label", t.hero.cta_primary);
  }
  ...
  function renderBoost() {
    ...
    setText(".boost-badge-title", t.boost.badge_title);
    setText(".boost-badge-text", t.boost.badge_text);
  ...
  function renderDiagnostic() {
    ...
    setText(".form-intelligence__label", t.diagnostic.smart_label);
    setText("#form-smart-hint", t.diagnostic.smart_hint);

    const trustItems = document.querySelectorAll(".diagnostic-trust .trust-item");
    if (trustItems.length && Array.isArray(t.diagnostic.trust)) {
      trustItems.forEach((item, index) => {
        if (t.diagnostic.trust[index]) item.textContent = t.diagnostic.trust[index];
      });
    }
  ...
  function applyFloatingButtonConfig() {
    ...
    const stickyTextValue = getLocalizedText(stickyCfg.text, "");
    const stickyHoverValue = getLocalizedText(stickyCfg.hoverText, "");
    const scrollTextValue = getLocalizedText(scrollCfg.text, "");
    const scrollHoverValue = getLocalizedText(scrollCfg.hoverText, "");
    ...
    if (stickyBtn) {
      ...
      stickyBtn.href = stickyCfg.link || fallbackWhatsApp;
    ...
    if (stickyText && stickyTextValue) stickyText.textContent = stickyTextValue;
    if (stickyTooltip && stickyHoverValue) stickyTooltip.textContent = stickyHoverValue;
    ...
    if (scrollBtn) {
      scrollBtn.href = scrollCfg.link || stickyCfg.link || fallbackWhatsApp;
      scrollBtn.target = "_blank";
      scrollBtn.rel = "noopener noreferrer";
      if (scrollTextValue) scrollBtn.setAttribute("aria-label", `${scrollTextValue} pelo WhatsApp`);
    }
    ...
    if (scrollText && scrollTextValue) scrollText.textContent = scrollTextValue;
    if (scrollTooltip) scrollTooltip.textContent = scrollHoverValue || stickyHoverValue || "";
  }
  ...
  function applyBackToTopConfig() {
    ...
    btn.setAttribute("aria-label", getLocalizedText(cfg.label, "Voltar ao topo"));
  ...
  function updateSmartHint(fieldName) {
    ...
    const localizedHints = getLocalizedText(cfg.hints, null);
    const fallback = t.diagnostic?.smart_hint || "Preencha os dados principais para receber um atendimento mais preciso.";
    hintEl.textContent = t.diagnostic?.hints?.[fieldName] || localizedHints?.[fieldName] || cfg.hints?.[fieldName] || fallback;
  }
  ...
  function validateFormData(data) {
    ...
    const messages = t.diagnostic?.validation || {};
    const selectWords = ["selecione", "selecionar", "select"];

    if (!data.name) {
      return { valid: false, field: "name", message: messages.name || `${labels.name || "Nome"} é obrigatório.` };
    }

    const phoneDigits = data.whatsapp.replace(/\D/g, "");
    if (phoneDigits.length < 10) {
      return { valid: false, field: "whatsapp", message: messages.whatsapp || "Informe um WhatsApp válido com DDD." };
    }

    if (!data.improve || selectWords.some((word) => data.improve.toLowerCase().includes(word))) {
      return { valid: false, field: "improve", message: messages.improve || "Escolha o que você deseja melhorar." };
    }

    if (!data.interest || selectWords.some((word) => data.interest.toLowerCase().includes(word))) {
      return { valid: false, field: "interest", message: messages.interest || "Escolha o interesse principal." };
    }
  ...
  function applyDataI18n() {
    document.querySelectorAll("[data-i18n]").forEach((el) => {
      const value = getTranslationValue(el.dataset.i18n);
      if (isSimpleText(value)) el.textContent = value;
    });

    document.querySelectorAll("[data-i18n-aria]").forEach((el) => {
      const value = getTranslationValue(el.dataset.i18nAria);
      if (isSimpleText(value)) el.setAttribute("aria-label", value);
    });

    document.querySelectorAll("[data-i18n-title]").forEach((el) => {
      const value = getTranslationValue(el.dataset.i18nTitle);
      if (isSimpleText(value)) el.setAttribute("title", value);
    });

    document.querySelectorAll("[data-i18n-alt]").forEach((el) => {
      const value = getTranslationValue(el.dataset.i18nAlt);
      if (isSimpleText(value)) el.setAttribute("alt", value);
    });
  }

  function getTranslationValue(path) {
    const fromLanguage = getPathValue(t, path);
    if (fromLanguage !== undefined) return fromLanguage;
    return getLocalizedText(getPathValue(CherryConfig, path), undefined);
  }
  ...
  function getLocalizedText(value, fallback = "") {
    ...
    return value[currentLang] || value[CherryConfig.defaultLang] || value.pt || fallback;
  }
